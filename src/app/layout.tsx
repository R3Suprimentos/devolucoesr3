    ...
    <html lang="pt-br" className="h-full w-full">
      <body className="flex flex-col h-full w-full min-h-screen min-w-0 scrollbar-dark">
        <AuthProvider>
          ...
          <Toaster />
          <div
            id="zim-messageWidget"
            style={{display:'flex',alignItems:'center',cursor:'pointer',width:'auto',position:'fixed',bottom:'24px',right:'24px',zIndex:9999}}
            onClick={() => window.open('https://wa.me/556231426268?text=Preciso%20de%20atendimento%20no%20sistema%20de%20Devolu%C3%A7%C3%B5es%20R3')}
          >
            <div style={{display:'flex',alignItems:'center',maxWidth:'100%',border:'1px solid transparent',boxSizing:'border-box',borderRadius:'50px',minHeight:'50px',padding:'13px 18px',backgroundColor:'#21D366',borderColor:'#21D366',color:'#fff',boxShadow:'0 4px 12px rgba(0,0,0,0.3)'}}>
              ...
              <div style={{wordBreak:'break-word',fontSize:'16px',marginLeft:'7px'}}>Suporte Ztech</div>
            </div>
          </div>
        </AuthProvider>
      </body>
